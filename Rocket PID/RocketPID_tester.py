...
class PressurePDTestCase(unittest.TestCase):
    ...
    def run_with_params(self, params: Dict):
        ...
        data = {'ErrorP': p[0],
                'ErrorI': 0,
                'ErrorD': p[1]}
        score, output = PressurePD().start_launch(target_pressure, student_pressure_pd_solution, optional_data = data)
        besterr = score
...
class RocketPIDTestCase(unittest.TestCase):
    ...
    def run_with_params(self, params: Dict):
        ...
        data = {'ErrorP': p[0],
                'ErrorI': p[1],
                'ErrorD': p[2]}
        (score, landing_score, flight_score,output) = RocketPID().launch_rocket(flight_plan, fuel, student_rocket_pid_solution, optional_data = data)
        besterr = score
        while sum(dp) > 0.00001:
            for i in range(len(p)):
                p[i] += dp[i]
                ...
                if err > besterr:
                    ...
                    if err > besterr:
                        ...
                        p[i] += dp[i]
                        dp[i] *= 0.9 """
        (score, landing_score, flight_score,output) = RocketPID().launch_rocket(flight_plan, fuel, student_rocket_pid_solution)
...
class BiPropellantPIDTestCase(unittest.TestCase):
    ...
    def run_with_params(self, params: Dict):
        ...
        data = {'ErrorP': p[0],
                'ErrorI': p[1],
                'ErrorD': p[2]}
        (score, landing_score, flight_score,output) = BiPropellantRocketPID().launch_rocket(flight_plan, fuel, oxidizer, student_bipropellant_pid_solution, optional_data = data)
        besterr = score
        while sum(dp) > 0.00001:
            for i in range(len(p)):
                p[i] += dp[i]
                ...
                if err > besterr:
                    ...
                    if err > besterr:
                        ...
                        p[i] += dp[i]
                        dp[i] *= 0.9 """
        (score, landing_score, flight_score,output) = BiPropellantRocketPID().launch_rocket(flight_plan, fuel, oxidizer, student_bipropellant_pid_solution)
...
def run_all(stream):
    suites = map(lambda case: unittest.TestSuite(unittest.TestLoader().loadTestsFromTestCase(case)),
                 [PressurePDTestCase, RocketPIDTestCase, BiPropellantPIDTestCase]) #, PressurePDTestCase, BiPropellantPIDTestCase])RocketPIDTestCase
